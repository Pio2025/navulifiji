...
class SeedPhysicalEducationCategoryAndLinkSubjects extends Migration
{
    public function up(): void
    {
        ...
        $existing = $db->table('subject_category')
            ->where('sub_cat_name', 'Physical Education')
            ->get()->getRowArray();

        if (!$existing) {
            $db->table('subject_category')->insert([
                'sub_cat_name'   => 'Physical Education',
                'sub_cat_status' => 1,
                'created_date'   => $today,
                'updated_date'   => $today,
            ]);
        ...
        $db->query(
            ...
            [$catId, '^Year [0-9]+ Physical Education$']
        );
    }
