...
class SeedCreativeArtsCategoryAndLinkSubjects extends Migration
{
    public function up(): void
    {
        ...
        $existing = $db->table('subject_category')
            ->where('sub_cat_name', 'Creative Arts')
            ->get()->getRowArray();

        if (!$existing) {
            $db->table('subject_category')->insert([
                'sub_cat_name'   => 'Creative Arts',
                'sub_cat_status' => 1,
                'created_date'   => $today,
                'updated_date'   => $today,
            ]);
        ...
        $db->table('subject')
            ->where('subject_name', 'Creative Arts')
