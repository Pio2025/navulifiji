...
class SeedSocialEmotionalDevelopmentCategoryAndLinkSubjects extends Migration
{
    public function up(): void
    {
        ...
        $existing = $db->table('subject_category')
            ->where('sub_cat_name', 'Social and Emotional Development')
            ->get()->getRowArray();

        if (!$existing) {
            $db->table('subject_category')->insert([
                'sub_cat_name'   => 'Social and Emotional Development',
                'sub_cat_status' => 1,
                'created_date'   => $today,
                'updated_date'   => $today,
            ]);
        ...
        $db->table('subject')
            ->where('subject_name', 'Social and Emotional Development')
