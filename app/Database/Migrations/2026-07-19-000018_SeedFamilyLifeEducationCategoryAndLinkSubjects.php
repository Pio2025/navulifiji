...
class SeedFamilyLifeEducationCategoryAndLinkSubjects extends Migration
{
    public function up(): void
    {
        ...
        $existing = $db->table('subject_category')
            ->where('sub_cat_name', 'Family Life Education')
            ->get()->getRowArray();

        if (!$existing) {
            $db->table('subject_category')->insert([
                'sub_cat_name'   => 'Family Life Education',
                'sub_cat_status' => 1,
                'created_date'   => $today,
                'updated_date'   => $today,
            ]);
        ...
        $db->query(
            ...
            [$catId, '^Year [0-9]+ Family Life Education$']
        );
    }
