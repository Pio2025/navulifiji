	...
	<body id="kt_body" data-bs-spy="scroll" data-bs-target="#kt_landing_menu" class="bg-body position-relative app-blank">
		...
		<script>
		    $("#kt_daterangepicker_3").daterangepicker({
                ...
                startDate: false,      // Prevents default date
                autoUpdateInput: false // Prevents auto-update (we'll handle it manually)
            }, function(start, end, label) {
                // This callback runs when date is selected
                if (start) {
                    // Format and set the selected date
                    var selectedDate = start.format('MM/DD/YYYY');
                    $("#kt_daterangepicker_3").val(selectedDate);
