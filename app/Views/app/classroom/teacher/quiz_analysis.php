...
<div id="kt_app_content_container" class="app-container container-xxl">
        ...
        <div class="card-body py-4 px-6 d-flex align-items-center gap-4 flex-wrap">
            <div class="symbol symbol-45px">
                <div class="symbol-label bg-light-<?= $statusColor ?>">
                    <i class="ki-duotone ki-questionnaire-tablet fs-3 text-<?= $statusColor ?>"><span class="path1"></span><span class="path2"></span></i>
                </div>
            </div>
            <div class="flex-grow-1">
                ...
                <div class="text-muted fs-8">
                    ...
                    &bull; <?= $totalQuestions ?> question<?= $totalQuestions !== 1 ? 's' : '' ?>
    ...
    <div class="row g-5 mb-6">
        ...
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body p-5 text-center">
                    <div class="fw-bold text-success fs-3x mb-1"><?= $submittedCount ?></div>
                    <div class="fw-semibold text-gray-700 fs-7 mb-1">Submitted</div>
                    <div class="fw-bold text-warning fs-4 mt-1"><?= $timedOutCount ?></div>
                    <div class="text-muted fs-9">Timed Out</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-5 mb-6">
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header border-0 pt-5 pb-0">
                    <h5 class="fw-bold text-gray-800 fs-6 mb-0">Submission Type</h5>
                    <span class="text-muted fs-9">Completed vs timed out</span>
                </div>
                <div class="card-body d-flex flex-column align-items-center justify-content-center pt-4">
                    <?php if ($totalAttempts > 0): ?>
                    <div id="chart_submission" style="min-height:220px;width:100%;"></div>
                    <div class="d-flex justify-content-center gap-5 mt-3">
                        <div class="d-flex align-items-center gap-2">
                            <span style="width:12px;height:12px;border-radius:50%;background:#50cd89;display:inline-block;"></span>
                            <span class="fs-8 text-muted">Submitted (<?= $submittedCount ?>)</span>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                            <span style="width:12px;height:12px;border-radius:50%;background:#ffc700;display:inline-block;"></span>
                            <span class="fs-8 text-muted">Timed Out (<?= $timedOutCount ?>)</span>
                        </div>
                    </div>
                    <?php else: ?>
                    <div class="text-center py-10 text-muted">
                        ...
                        <div class="fs-7 fw-semibold">No attempts yet</div>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <!--begin::Per-question table-->
    <div class="card border-0 shadow-sm mb-6" style="overflow:hidden;">
        <div class="card-header border-0 pt-5 pb-4">
            <h5 class="fw-bold text-gray-800 fs-6 mb-0">Per-Question Performance</h5>
            <span class="text-muted fs-9"><?= $totalQuestions ?> question<?= $totalQuestions !== 1 ? 's' : '' ?> &mdash; up to 20 questions supported</span>
        </div>
        ...
        <div class="card-body p-0">
            ...
            <div class="text-center py-12 text-muted">
                ...
                <div class="fs-7 fw-semibold">No questions in this quiz yet.</div>
            </div>
            <?php else: ?>
                ...
                <table class="table table-row-dashed table-row-gray-200 align-middle mb-0" style="min-width:600px;">
                    <thead>
                        <tr class="fw-bold text-muted fs-8" style="background:#f9f9f9;">
                            ...
                            <th class="py-4">Question</th>
                            <th class="text-center w-90px py-4">Answers</th>
                    ...
                    <?php foreach ($questionStats as $q):
                        $barColor  = $q['pct'] >= 70 ? '#50cd89' : ($q['pct'] >= 50 ? '#009ef7' : '#f1416c');
                        $textColor = $q['pct'] >= 70 ? 'text-success' : ($q['pct'] >= 50 ? 'text-info' : 'text-danger');
                    ?>
                    <tr>
                        <td class="ps-7 py-4">
                            <div class="symbol symbol-35px">
                                <div class="symbol-label bg-light-primary fw-bold text-primary fs-8">Q<?= $q['number'] ?></div>
                            </div>
                        </td>
                        <td class="fw-semibold text-gray-800 fs-7 py-4" style="max-width:360px;">
                            <?= esc($q['question']) ?>
                        </td>
                        <td class="text-center fw-bold text-gray-700 fs-7 py-4"><?= $q['total'] ?></td>
                        <td class="text-center fw-bold text-success fs-7 py-4"><?= $q['correct'] ?></td>
                        <td class="text-center fw-bold text-danger fs-7 py-4"><?= $q['incorrect'] ?></td>
                        <td class="pe-7 py-4">
                            <div class="d-flex align-items-center gap-2">
                                <div class="flex-grow-1" style="height:8px;background:#f1f1f4;border-radius:4px;">
                                    <div style="height:8px;width:<?= $q['pct'] ?>%;background:<?= $barColor ?>;border-radius:4px;transition:width .5s;"></div>
                                </div>
                                <span class="fw-bold <?= $textColor ?> fs-8 flex-shrink-0" style="min-width:40px;text-align:right;"><?= $q['pct'] ?>%</span>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
    ...
    <!--end::Per-question table-->

    <!--begin::Attempts list (if any)-->
    <?php if ($totalAttempts > 0): ?>
    <div class="card border-0 shadow-sm mb-6" style="overflow:hidden;">
        <div class="card-header border-0 pt-5 pb-4">
                ...
                <table class="table table-row-dashed table-row-gray-200 align-middle gs-0 gy-3 mb-0" id="attempts_table">
                    <thead>
                        <tr class="fw-bold text-muted fs-8" style="background:#f9f9f9;">
                            ...
                            <th class="text-center w-120px py-3">Status</th>
                            <th class="text-center w-130px py-3">Correct / Total</th>
                            <th class="pe-0 w-160px py-3">Submitted</th>
                        </tr>
                    </thead>
                    <tbody id="attempts_tbody">
                        <tr><td colspan="5" class="text-center py-6"><span class="spinner-border spinner-border-sm text-primary"></span></td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
const SUBMITTED_CNT  = <?= (int) $submittedCount ?>;
const TIMEDOUT_CNT   = <?= (int) $timedOutCount ?>;
const TOTAL_ATTEMPTS = <?= (int) $totalAttempts ?>;
const QUIZ_ID        = <?= (int) $quizzeId ?>;
const LESSON_ID      = <?= (int) $lessonId ?>;
const ATTEMPTS_URL   = '<?= base_url("classroom/teacher/{$schSubId}/lesson/{$lessonId}/quiz/{$quizzeId}/attempts") ?>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ── Submission type donut ─────────────────────────────────────
    if (TOTAL_ATTEMPTS > 0) {
        new ApexCharts(document.getElementById('chart_submission'), {
            series: [SUBMITTED_CNT, TIMEDOUT_CNT],
            chart: { type: 'donut', height: 220 },
            labels: ['Submitted', 'Timed Out'],
            ...
            plotOptions: { pie: { donut: { size: '65%',
                labels: { show: true, total: { show: true, label: 'Total', fontSize: '13px',
                    formatter: () => TOTAL_ATTEMPTS + ' attempt' + (TOTAL_ATTEMPTS !== 1 ? 's' : '') } } } } },
            stroke: { width: 2 }
        }).render();
    }
});
...
$.get(ATTEMPTS_URL, function(res) {
    ...
    const rows = res.attempts.map(function(a) {
        ...
        return `<tr>
            ...
            <td class="text-center text-muted fs-8">${a.correct_answers} / ${a.total_questions}</td>
