...
<div id="kt_app_content_container" class="app-container container-xxl">
        ...
        <div class="card-body py-4 px-6 d-flex align-items-center gap-4 flex-wrap">
            <div class="symbol symbol-45px">
                <div class="symbol-label bg-light-info">
                    <i class="ki-duotone ki-tag fs-3 text-info"><span class="path1"></span><span class="path2"></span><span class="path3"></span></i>
                </div>
            </div>
            <div class="flex-grow-1">
                ...
                <div class="text-muted fs-8">
                    ...
                    &bull; <?= $totalMarkers ?> marker<?= $totalMarkers !== 1 ? 's' : '' ?>
    ...
    <div class="row g-5 mb-6">
        ...
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body p-5 text-center">
                    <div class="fw-bold text-info fs-2x mb-1"><?= $totalMarkers ?></div>
                    <div class="fw-semibold text-gray-700 fs-7 mb-1">Total Markers</div>
                    <div class="text-muted fs-9">labels to identify on image<?= count($markerStats) !== $totalMarkers ? 's' : '' ?></div>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="card-body d-flex flex-column align-items-center justify-content-center pt-4">
                    ...
                    <div class="d-flex justify-content-center gap-5 mt-3">
                        <div class="d-flex align-items-center gap-2">
                            <span style="width:12px;height:12px;border-radius:50%;background:#7239ea;display:inline-block;"></span>
    ...
    <!--begin::Per-marker table-->
    <div class="card border-0 shadow-sm mb-6" style="overflow:hidden;">
        <div class="card-header border-0 pt-5 pb-4">
            <h5 class="fw-bold text-gray-800 fs-6 mb-0">Per-Marker Performance</h5>
            <span class="text-muted fs-9"><?= $totalMarkers ?> marker<?= $totalMarkers !== 1 ? 's' : '' ?> &mdash; how often each label was correctly identified</span>
        </div>
        <div class="separator separator-dashed mx-6 mb-0"></div>
        <div class="card-body p-0">
            <?php if (empty($markerStats)): ?>
            <div class="text-center py-12 text-muted">
                <i class="ki-duotone ki-tag fs-4x text-gray-200 mb-3"><span class="path1"></span><span class="path2"></span><span class="path3"></span></i>
                <div class="fs-7 fw-semibold">No markers in this assessment yet.</div>
            </div>
            <?php else:
                $lastQuestion = null;
            ?>
            <div class="table-responsive">
                <table class="table table-row-dashed table-row-gray-200 align-middle mb-0" style="min-width:620px;">
                    <thead>
                        <tr class="fw-bold text-muted fs-8" style="background:#f9f9f9;">
                            <th class="ps-7 w-60px py-4">Marker</th>
                            <th class="py-4">Correct Label</th>
                    ...
                    <tbody>
                    <?php foreach ($markerStats as $m):
                        $barColor  = $m['pct'] >= 70 ? '#50cd89' : ($m['pct'] >= 50 ? '#009ef7' : '#f1416c');
                        $textColor = $m['pct'] >= 70 ? 'text-success' : ($m['pct'] >= 50 ? 'text-info' : 'text-danger');
                    ?>
                    <?php if ($m['question_text'] !== $lastQuestion): $lastQuestion = $m['question_text']; ?>
                    <tr class="bg-light-secondary">
                        <td colspan="6" class="ps-7 py-2">
                            <span class="badge badge-light-info me-2 fs-9">Q<?= $m['question_num'] ?></span>
                            <span class="fw-semibold text-gray-700 fs-8"><?= esc($m['question_text'] ?: 'Image ' . $m['question_num']) ?></span>
                        </td>
                    </tr>
                    <?php endif; ?>
                    <tr>
                        <td class="ps-7 py-4">
                            <div class="symbol symbol-35px">
                                <div class="symbol-label bg-light-info fw-bold text-info fs-8">M<?= $m['marker_num'] ?></div>
                            </div>
                        </td>
                        <td class="fw-semibold text-gray-800 fs-7 py-4" style="max-width:260px;">
                            <span class="badge badge-light-success fs-9"><?= esc($m['correct_label']) ?></span>
                        </td>
                        <td class="text-center fw-bold text-gray-700 fs-7 py-4"><?= $m['total'] ?></td>
                        <td class="text-center fw-bold text-success fs-7 py-4"><?= $m['correct'] ?></td>
                        <td class="text-center fw-bold text-danger fs-7 py-4"><?= $m['incorrect'] ?></td>
                        <td class="pe-7 py-4">
                            <div class="d-flex align-items-center gap-2">
                                <div class="flex-grow-1" style="height:8px;background:#f1f1f4;border-radius:4px;">
                                    <div style="height:8px;width:<?= $m['pct'] ?>%;background:<?= $barColor ?>;border-radius:4px;transition:width .5s;"></div>
                                </div>
                                <span class="fw-bold <?= $textColor ?> fs-8 flex-shrink-0" style="min-width:40px;text-align:right;"><?= $m['pct'] ?>%</span>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                ...
                <table class="table table-row-dashed table-row-gray-200 align-middle gs-0 gy-3 mb-0" id="attempts_table">
                    <thead>
                        <tr class="fw-bold text-muted fs-8" style="background:#f9f9f9;">
                            ...
                            <th class="text-center w-160px py-3">Correct / Total Markers</th>
...
const ATTEMPTS_URL   = '<?= base_url("classroom/teacher/{$schSubId}/lesson/{$lessonId}/label/{$quizzeId}/attempts") ?>';
...
$.get(ATTEMPTS_URL, function(res) {
    ...
    const rows = res.attempts.map(function(a) {
        ...
        return `<tr>
            ...
            <td class="text-center text-muted fs-8">${a.correct_markers} / ${a.total_markers}</td>
