...
<div id="kt_app_content_container" class="app-container container-xxl">
    ...
    <?php if (!empty($isPrincipal)): ?>
    <?= view('app/classroom/principal/_pending_reports', ['pendingExamReports' => $pendingExamReports ?? []]) ?>
    <?php endif; ?>

    <!--begin::Year classroom content-->
    <div id="tch-year-content">
        <?= view('app/classroom/teacher/_year_classrooms', [
...
<script>
var _tchYearAjaxUrl = '<?= base_url('classroom/my/year/') ?>';

function tmyOpenChat(userId, fname, lname, photo) {
...
function _tchInjectAndExec(container, html) {
...
(function () {
    var container  = document.getElementById('tch-year-content');
    var activeYear = '<?= $defaultYear ?? '' ?>';
    var YEAR_KEY   = 'tmy_year';
    ...
    function loadYear(yr, callback) {
        ...
        fetch(_tchYearAjaxUrl + yr, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(function (r) { return r.json(); })
            .then(function (data) {
                _tchInjectAndExec(container, data.html || '');
                activeYear = yr;
