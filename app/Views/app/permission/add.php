...
<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
	<div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
		<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
			<h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">Add Permission</h1>
...
<div id="kt_app_content" class="app-content flex-column-fluid">
	<div id="kt_app_content_container" class="app-container container-xxl">
		...
		<div class="card">
			...
			<div class="card-body">
				<!--begin::Form-->
				<form id="kt_permission_add_form" action="<?= base_url('permission/store') ?>" method="POST">
					<?= csrf_field() ?>
							...
							<select name="module_id_fk" id="module_id_fk" class="form-select  <?= session()->has('validation') && session('validation')->hasError('module_id_fk') ? 'is-invalid' : '' ?>">
								...
								<?php if (!empty($modules)): ?>
									<?php foreach ($modules as $module): ?>
										<option value="<?= $module['module_id'] ?>" <?= old('module_id_fk') == $module['module_id'] ? 'selected' : '' ?>>
					...
					<div class="row mb-6">
						...
						<div class="col-lg-8">
							<textarea name="perm_desc" 
							          ...
							          rows="3" 
							          placeholder="Describe what this permission allows..."><?= old('perm_desc') ?></textarea>
					...
					<div class="row mb-6">
						...
						<div class="col-lg-8">
							<select name="perm_status" id="perm_status" class="form-select ">
								<option value="Active" <?= old('perm_status', 'Active') === 'Active' ? 'selected' : '' ?>>Active</option>
								<option value="Inactive" <?= old('perm_status') === 'Inactive' ? 'selected' : '' ?>>Inactive</option>
							</select>
						</div>
					</div>
					<!--end::Input group-->
...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('kt_permission_add_form');
    const permNameInput = document.getElementById('perm_name');
    const permCodeInput = document.getElementById('perm_code');
    
    // Auto-generate permission code from permission name
    if (permNameInput && permCodeInput) {
        permNameInput.addEventListener('input', function() {
            if (!permCodeInput.value || permCodeInput.dataset.manual !== 'true') {
                const code = '_' + this.value
                    .toLowerCase()
                    .replace(/[^a-z0-9\s]/g, '')
                    .replace(/\s+/g, '_');
                permCodeInput.value = code;
            }
        });
        
        // Mark as manually edited if user types in code field
        permCodeInput.addEventListener('input', function() {
