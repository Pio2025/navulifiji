...
<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
    <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">Add School Category</h1>
...
<div id="kt_app_content" class="app-content flex-column-fluid">
    <div id="kt_app_content_container" class="app-container container-xxl">
        ...
        <?php $oldInput = session()->getFlashdata('_ci_old_input') ?? []; ?>

        <form id="school_cat_form" action="<?= base_url('school/category/store') ?>" method="POST">
            <?= csrf_field() ?>
                    ...
                    <div class="row g-5">
                        <div class="col-lg-6">
                            ...
                            <select name="num_of_term_in_year" id="num_of_term"
                                ...
                                <?php for ($n = 1; $n <= 12; $n++): ?>
                                    <option value="<?= $n ?>" <?= old('num_of_term_in_year') == $n ? 'selected' : '' ?>><?= $n ?></option>
                                <?php endfor; ?>
                            </select>
                            <div class="form-text text-muted">Selecting generates the term entry cards below</div>
                        ...
                        <div class="col-lg-6">
                            ...
                            <input type="text" name="label_for_term" id="label_for_term_input"
                                ...
                                value="<?= esc(old('label_for_term', 'Term')) ?>" />
...
(function () {
    ...
    function buildTermEntries(n) {
        ...
        for (var i = 1; i <= n; i++) {
            container.innerHTML +=
                ...
                weeksInput('num_of_week_' + i, oldInput['num_of_week_' + i] || '') +
                '</div>' +
                ...
                '<div class="d-flex align-items-center gap-2">' +
                dayInput('term_start_day_' + i, oldInput['term_start_day_' + i] || '') +
                monthSelect('term_start_month_' + i, oldInput['term_start_month_' + i] || '') +
                '</div></div>' +
                ...
                '<div class="d-flex align-items-center gap-2">' +
                dayInput('term_end_day_' + i, oldInput['term_end_day_' + i] || '') +
                monthSelect('term_end_month_' + i, oldInput['term_end_month_' + i] || '') +
                '</div></div>' +
    ...
    var oldNum = parseInt(numOfTermSel.value) || 0;
    if (oldNum > 0) buildTermEntries(oldNum);

    document.getElementById('school_cat_form').addEventListener('submit', function (e) {
        var n = parseInt(numOfTermSel.value) || 0;
