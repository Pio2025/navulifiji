...
/**
 * Session-agnostic dashboard stat queries, extracted from DashboardController
 * so both the session-based web controller and the JWT-based mobile API
 * controller can share the exact same query logic.
 */
class DashboardStats
{
    protected ParentStudentModel $parentStudentModel;
    protected TermExamModel $termExamModel;
    ...
    public function schoolAdminStats(int $schId): array
    {
        $db = \Config\Database::connect();
    ...
    public function teacherStats(int $userId): array
    {
        $db = \Config\Database::connect();
    ...
    public function parentStats(int $userId): array
    {
        $db = \Config\Database::connect();
